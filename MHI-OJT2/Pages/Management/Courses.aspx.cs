...
namespace MHI_OJT2.Pages.Management
{
    public partial class Courses : System.Web.UI.Page
	{
		...
		public static string GetEmployeeList(int courseId)
		{
			...
			SqlCommand command = new SqlCommand("SELECT TOP 50 p.PersonID ,CONCAT(p.PersonCode, SPACE(8 - LEN(p.PersonCode)), ' ', p.FnameT, ' ', p.LnameT, '  (', Cmb2NameT ,')') fullName, IIF(e.PERSON_ID = p.PersonID, 1, 0) as selected FROM Cyberhrm_PCT.dbo.PNT_Person p INNER JOIN Cyberhrm_PCT.dbo.PNM_Cmb2 c ON c.Cmb2ID = p.Cmb2ID LEFT JOIN TEST_DB.dbo.EVALUATE e ON e.PERSON_ID = p.PersonID AND e.COURSE_ID = @courseId", con);
			con.Open();
