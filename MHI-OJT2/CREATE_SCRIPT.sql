            ...
            FROM 
             (
                SELECT 
                    ...
                    ,POSITION_NAME_TH
                    ,COURSE_NAME
                    ,(SELECT COUNT(*) FROM COURSE_AND_EMPLOYEE c WHERE c.PersonID = COURSE_AND_EMPLOYEE.PersonID AND (SELECT COUNT(*) FROM PLAN_AND_COURSE p WHERE p.COURSE_ID = c.COURSE_ID) > 0) PLAN_A
                    ,(SELECT COUNT(CASE WHEN TOTAL_SCORE > 50 THEN 1 END) FROM COURSE_AND_EMPLOYEE c WHERE c.PersonID = COURSE_AND_EMPLOYEE.PersonID AND (SELECT COUNT(*) FROM PLAN_AND_COURSE p WHERE p.COURSE_ID = c.COURSE_ID) > 0) PLAN_B
                    ,TOTAL_SCORE
                FROM COURSE_AND_EMPLOYEE
